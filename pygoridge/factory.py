...
CONNECTION_PATTERN = re.compile(f"({Relay.STREAM}|(?P<protocol>({Relay.TCP_SOCKET}|{Relay.UNIX_SOCKET})):\/\/(?P<address>[^:]+)(:(?P<port>[^:]+))?)", re.IGNORECASE)


def create_relay(connection: str) -> Relay:
    m = CONNECTION_PATTERN.match(connection)
    if not m:
        raise RelayFactoryException(f"unknown connection specification: {connection}")
    ...
    if m.string == Relay.STREAM:
        ...
        return SocketRelay(m.group("address"), port=m.group("port"), 
